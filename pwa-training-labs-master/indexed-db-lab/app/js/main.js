...
var idbApp = (function() {
  ...
  var dbPromise = idb.open('couches-n-things', 5, function(upgradeDb) {
    ...
    switch (upgradeDb.oldVersion) {
      ...
      case 3:
        console.log('Creating price and description indices');
        var store = upgradeDb.transaction.objectStore('products');
  ...
  function addOrders() {
    dbPromise.then(function(db) {
      ...
      return Promise.all(
        items.map(function(item) {
          console.log('Adding orders: ', item);
          return store.add(item);
        })
      )
        .catch(function(e) {
  ...
  function showOrders() {
    ...
    dbPromise
      .then(function(db) {
        // 5.3 - use a cursor to display the orders on the page
  ...
  function processOrders(orders) {
    // 5.5 - get items in the 'products' store matching the orders
    return dbPromise.then(function(db) {
  ...
  function decrementQuantity(product, order) {
    // 5.6 - check the quantity of remaining products
      ...
      .then(function(db) {
        ...
        return Promise.all(
          products.map(function(product) {
            console.log('Updating item:', product.name);
